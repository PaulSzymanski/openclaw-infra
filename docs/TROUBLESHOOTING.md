...
**Symptom**: `./scripts/provision.sh` fails with "gateway_token is empty" or "claude_setup_token is empty".

During `pulumi up`, secrets are passed automatically via environment variables. For manual day-2 runs, `provision.sh` reads from Pulumi CLI. Fix depends on which secret is missing:

```bash
cd pulumi

# If claude_setup_token is empty — regenerate and set:
pulumi config set claudeSetupToken --secret

# If gateway_token is empty — it's auto-generated by Pulumi:
pulumi up   # regenerates the token
```
...
### Sessions fail with Docker permission denied

**Symptom**: Sessions fail with "permission denied" when accessing Docker.

After adding ubuntu to the docker group (e.g., during provisioning), the systemd user manager must be restarted for the gateway to inherit the new group. Provisioning handles this automatically, but if you manually change docker group membership:

```bash
ssh ubuntu@openclaw-vps.<tailnet>.ts.net \
  'sudo systemctl restart user@1000.service && sleep 2 && XDG_RUNTIME_DIR=/run/user/1000 systemctl --user restart openclaw-gateway'
```

### Custom Docker image build failed

**Symptom**: Sessions fail with "docker: Error response from daemon" or image not found.

The sandbox uses two locally-built images. Check both exist:
```bash
ssh ubuntu@openclaw-vps.<tailnet>.ts.net 'docker image ls | grep openclaw-sandbox'
```

Expected: `openclaw-sandbox` (base) and `openclaw-sandbox-custom` (custom). If missing, rebuild:
```bash
